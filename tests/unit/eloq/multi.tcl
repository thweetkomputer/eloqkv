...
# TODO: transaction conflict with flushdb/flushall. issue tracked.
    ...
    test {SWAPDB is able to touch the watched keys that exist} {
        ...
        r ping
        r exec
    } {} {singledb:skip needs:support_cmd_later}
    ...
    test {SWAPDB does not touch watched stale keys} {
        ...
        assert_equal {PONG} [r exec]
        r debug set-active-expire 1
    } {OK} {singledb:skip needs:debug needs:support_cmd_later}
    ...
    test {exec with write commands and state change} {
        ...
        $r1 close
    } {0} {needs:repl needs:redis_config}
    ...
    test {exec with read commands and stale replica state change} {
        ...
        assert { $xx == 1 }
        $r1 close
    } {0} {needs:repl cluster:skip needs:redis_config}
    ...
    tags {"stream"} {
        test {MULTI propagation of XREADGROUP} {
            ...
            close_replication_stream $repl
        } {} {needs:repl needs:support_cmd_later}
    }
    ...
    test "MULTI with config set appendonly" {
        ...
        waitForBgrewriteaof r
    } {} {external:skip needs:redis_config}
