start_server {tags {"slowlog"} overrides {slowlog-log-slower-than 1000000}} {
    test {SLOWLOG - check that it starts with an empty log} {
        if {$::external} {
            r config set slowlog-log-slower-than 10000000
            r slowlog reset
        }
        r slowlog len
    } {0}
    ...
    # We do not know when the blocking command is unblocked since it is directly
    # executed in cc map, so we skip this test
